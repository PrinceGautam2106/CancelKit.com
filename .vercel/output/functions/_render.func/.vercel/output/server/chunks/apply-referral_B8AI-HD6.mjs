...
function referralRewardHtml(data) {
	...
	const twitterShareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`I just earned a free month of @SubZaps for helping a friend cancel forgotten subscriptions 💸\n\nGet your first month free: ${referralUrl}`)}`;
	const body = `
    ...
    <p style="${styles.body}text-align:center;">
      Awesome news, ${firstName}! ${friend} just upgraded to SubZaps Pro. Because you invited them, you <strong style="color:#ffffff;">BOTH</strong> get 1 month of SubZaps Pro on us.
    </p>
  ...
  `;
	return emailLayout(body, {
		preheader: `You earned 1 free month of SubZaps Pro! $${data.dollarCredit} credit applied to your account.`,
		userId: data.userId,
		email: data.email
	});
}
	...
	try {
		...
		if (stripeCustomerId && !stripeCustomerId.startsWith("cus_mock")) try {
			await stripe.customers.createBalanceTransaction(stripeCustomerId, {
				amount: creditAmount,
				currency: "usd",
				description: "Referral reward: 1 month free SubZaps Pro"
			});
		} catch (stripeErr) {
		...
		if (referrerEmail) try {
			...
			const emailHtml = referralRewardHtml({
				...
				dollarCredit: "9.00",
				referralCode: referralCode || "subzap"
			});
