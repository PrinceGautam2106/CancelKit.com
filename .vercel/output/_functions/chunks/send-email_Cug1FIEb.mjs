...
import { n as paymentFailedSubject, t as paymentFailedHtml } from "./payment-failed_B70zHCmv.mjs";
...
function weeklySavingsHtml(data) {
	...
	const twitterShareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(`I've saved $${data.totalSaved} on subscriptions I forgot about using @SubZaps 💸\n\nStop paying for things you don't use → subzaps.com`)}`;
...
var POST = async ({ request }) => {
	...
	if (!(request.headers.get("x-api-secret") || origin && origin.includes("subzaps.com") || origin && origin.includes("localhost") || referer && referer.includes("subzaps.com") || referer && referer.includes("localhost"))) return new Response(JSON.stringify({ error: "Unauthorized" }), {
		status: 401,
